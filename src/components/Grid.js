...
function ComponentArray(props) {
  ...
  return (
    <div style={{alignItems: 'center' }}>
      {rowData.map((row, index) => (
        <div key={index} style={{ display: 'grid', width: '100%', backgroundColor: row.backGroundColor, alignItems: 'center', gridTemplateColumns: 'repeat(2, 1fr)' }}>
          <div style={{}}>{row.component1}</div>
          <div style={{}}>{row.component2}</div> 
        </div>
      ))}
    </div>
  );
